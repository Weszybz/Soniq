...
export async function sendTextMessage(conversationId, content) {
  ...
  try {
    const conv = await databases.getDocument(DATABASE_ID, CONVERSATIONS_COLLECTION_ID, conversationId);
    const recipientId = conv.participantIds.find((id) => id !== user.$id);
    if (recipientId) {
      createNotification({
        recipientId,
        senderId: user.$id,
  ...
  try {
    ...
    if (recipientId) {
      createNotification({
        ...
        type: 'message',
        conversationId,
        content: 'sent you an audio message',
      });
    }
  } catch (_) {}
  ...
  try {
    ...
    if (recipientId) {
      createNotification({
        ...
        content: `shared a snippet: "${snippet.title}"`,
      });
    }
  } catch (_) {}
