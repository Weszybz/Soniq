...
export async function ensureUserDocument(userId, userData) {
  try {
    ...
    try {
      ...
      if (existingUsers.documents.length > 0) {
        ...
        const updatePayload = {
          ...
          if (userData.genres !== undefined) {
            updatePayload.genres = userData.genres;
          }
    ...
    if (userData.genres !== undefined) {
      createPayload.genres = userData.genres;
    }
...
export async function searchUsersByUsername(query, limit = 8) {
  ...
  try {
    ...
    if (userDoc) {
      ...
      return {
        ...
        genres: Array.isArray(userDoc.genres) ? userDoc.genres : [],
      };
    }
