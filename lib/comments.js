...
export async function createComment({ snippetId, content, parentCommentId = null, timestamp = 0, snippetOwnerId = null, snippetTitle = null }) {
    try {
        ...
        if (snippetOwnerId && snippetOwnerId !== user.$id) {
            const username = user.prefs?.username || user.name || 'Someone';
            const profileImage = user.prefs?.profileImage || null;
            createNotification({
                recipientId: snippetOwnerId,
                senderId: user.$id,
                ...
                commentId: doc.$id,
                content: `commented on your snippet${snippetTitle ? ` "${snippetTitle}"` : ''}`,
            });
        }
